...
import { Patient, Branch, PageProps } from "@/types";
import {
    Pencil,
    Trash2,
    Plus,
...
export default function PatientIndex({ patients, branches }: Props) {
    ...
    const verifyPatient = async () => {
        ...
        try {
            ...
            if (response.data.patient) {
                setVerifiedPatient(response.data.patient);
            } else {
                setVerifyError(
                    "Patient not found with that phone number or card number.",
                );
            }
        } catch (error) {
    ...
    const openDelete = (patient: Patient) => {
        ...
        setDeleteDetails({ type: "branch", branch_id: "" }); // Default
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (showEditModal && selectedPatient) {
            ...
            post(route("patients.update", selectedPatient.id), {
                onSuccess: () => {
